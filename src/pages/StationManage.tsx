...
export function StationManage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteStation(deleteTarget.id, {
        confirm: false,
        stationName: deleteTarget.nome, // <-- Voltou para .nome (padrão do seu colega)
      });
      await reload();
      setDeleteTarget(null);
    } catch {
  ...
  // ==========================================
  // COLUNAS DA TABELA (Usando o padrão do Mapper)
  // ==========================================
  const colunasDaTabela: TableColumn<Estacao>[] = [
    {
      key: "nome",
      header: "NOME",
      render: (item) => <span className="font-semibold text-gray-900 uppercase">{item.nome}</span>,
    },
    {
      key: "cidade",
      header: "CIDADE",
      render: (item) => <span className="text-gray-600">{item.cidade || "-"}</span>,
    },
    {
      key: "codigo",
      header: "CÓDIGO",
      render: (item) => <span className="text-gray-600">{item.codigo || "-"}</span>,
    },
    {
      key: "isActive",
      header: "STATUS",
